...
from ucbs.config.certificate_spec import (
    ...
    CLAIM_FREEZE_MEMBER,
    EXPECTED_AGGREGATION_ROWS,
    EXPECTED_CLASS_COUNTS,
    EXPECTED_FINAL_BLOCKERS,
...
def check_final_adjudication(root: Path, archive: str | Path) -> ComponentReport:
    ...
    member_rows = require_members(path, list(FINAL_REQUIRED_MEMBERS))
    checks.extend(member_rows)
    if not all(truthy(row.get("passed")) for row in member_rows):
        return ComponentReport("final_adjudication", False, checks, {"final_adjudication_passed": False})

    certificate_status = read_json_member(path, CERTIFICATE_STATUS_MEMBER)
    final_status = read_json_member(path, FINAL_STATUS_MEMBER)
    theorem_gate = read_json_member(path, THEOREM_GATE_MEMBER)
    claim_freeze = read_json_member(path, CLAIM_FREEZE_MEMBER)
    ledger = read_csv_member(path, FINAL_LEDGER_MEMBER)
    aggregation = read_csv_member(path, FINAL_AGGREGATION_GATE_MEMBER)
    blockers = read_csv_member(path, FINAL_BLOCKER_MATRIX_MEMBER)
    ...
    checks.append(require_exact_count(blockers, EXPECTED_FINAL_BLOCKERS, "final_blocker_matrix"))
    ...
    checks.append(check_row("ledger_rows_pass", all_pass(ledger, "pass"), len(ledger), "all final ledger rows pass"))
    ...
    checks.append(check_row("aggregation_rows_pass", all_pass(aggregation, "passed"), len(aggregation), "all final aggregation gates pass"))
    ...
    for name, status in [
        ...
        checks.append(check_row(f"{name}_target", str(status.get("target")) == CERTIFIED_THRESHOLD_TEXT, status.get("target"), f"{name} records the certified target"))
